...
public class Unregister extends Action<String> {
    ...
    protected void start() {
        ...
        callback second = new callback() { // would fire after the course grade was removed from the student
            @Override
            public void call() {
                complete(studentName + " unregistered from " + courseName);
            }
        };
        callback first = new callback() { // after we waited for all the students , this callback would fire removing the student from the course and sending action which will remove the course grade from the student
            @Override
            public void call() {
